...
- 解決策を提示せずに分析結果のみを提示する
- 分析対象のリポジトリの技術スタックに依存しない（言語・フレームワークを仮定しない）
- コアとしての CLI ツールに AI/ML 機能を含めない

**根拠**: 分析と自動化を分離することで、ikura はあらゆるコードベースで信頼できる予測可能な指標ツールとして機能する。AI による提案は、ikura の出力を利用する別ツール (ikura-agent) の責務とする。

### II. CLI 中心

ikura の全ての機能はコマンドラインインターフェースからアクセス可能でなければならない。全 CLI ツールは以下を必須とする:

- コマンドライン引数および特定の toml ファイルから入力を受け付ける
- stdout（データ）と stderr（エラー・診断情報）に結果を出力する
- 人間が読める形式と JSON 出力形式の両方をサポート（`--format json`）
- 明確なヘルプテキストを提供（`--help`）
- 適切な終了ステータスコードで終了（0 = 成功、非ゼロ = エラー）

**根拠**: CLI 優先設計により、ikura は自動化ワークフロー、CI/CD パイプライン、他の Unix ツールとの組み合わせが可能になる。JSON を出力することで、Web コンソールや AI エージェントから読み取れるようになる。
...
3. **ikura-agent**: AI によるリファクタリング自動化 - ikura が出力した JSON を読み取り、リファクタリングの PR を作成

各コンポーネントは以下を必須とする:

- 独立して動作する（CLI は Web コンソールなしで動作、Web コンソールは ikura が出力する任意の JSON に対して動作）
- 明確に定義された JSON スキーマを通じて通信する
- 循環依存を避ける

**根拠**: 明確な境界により、独立した開発・テスト・デプロイが可能になる。ユーザーは必要なコンポーネントのみを採用できる。CLI は、monorepo を構成するパッケージ全てをインストールせずとも、任意のリポジトリで使用できる。
