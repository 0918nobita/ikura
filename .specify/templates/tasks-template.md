...
- 以下に示すパスはシングルプロジェクトを前提 - plan.md の構造に基づいて調整

<!--
  ============================================================================
  重要: 以下のタスクはサンプルタスクであり、説明用途のみです。
  ...
  タスクは各ストーリーが以下を可能にするようユーザーストーリーごとに整理する必要があります:
  - 独立して実装
  - 独立してテスト
  - MVP の増分として提供
...
- [ ] T008 エラーハンドリングとログインフラストラクチャを設定
- [ ] T009 環境設定管理をセットアップ

**チェックポイント**: 基盤が準備完了 - ユーザーストーリーの実装を並行して開始可能
...
> **注記: これらのテストを最初に書き、実装前に失敗することを確認してください**

- [ ] T010 [P] [US1] tests/contract/test_[name].py の [エンドポイント] の契約テスト
...
- [ ] T018 [P] [US2] tests/contract/test_[name].py の [エンドポイント] の契約テスト
...
- [ ] T024 [P] [US3] tests/contract/test_[name].py の [エンドポイント] の契約テスト
...
**チェックポイント**: すべてのユーザーストーリーが独立して機能するようになっているべき

---

[必要に応じて同じパターンに従って、さらにユーザーストーリーのフェーズを追加]
...
- [ ] TXXX [P] tests/unit/ の追加のユニットテスト (要求された場合)
- [ ] TXXX セキュリティ強化
...
- **ユーザーストーリー (フェーズ 3+)**: すべて基盤フェーズの完了に依存
  - ユーザーストーリーは並列で進行可能 (スタッフが十分な場合)
  - または優先順位順に順次 (P1 → P2 → P3)
- **仕上げ (最終フェーズ)**: すべての望ましいユーザーストーリーの完了に依存

### ユーザーストーリーの依存関係

- **ユーザーストーリー 1 (P1)**: 基盤 (フェーズ 2) の後に開始可能 - 他のストーリーへの依存関係なし
- **ユーザーストーリー 2 (P2)**: 基盤 (フェーズ 2) の後に開始可能 - US1 と統合する可能性があるが独立してテスト可能であるべき
- **ユーザーストーリー 3 (P3)**: 基盤 (フェーズ 2) の後に開始可能 - US1/US2 と統合する可能性があるが独立してテスト可能であるべき
...
- [P] とマークされたすべての基盤タスクは並列実行可能 (フェーズ 2 内)
- 基盤フェーズが完了すると、すべてのユーザーストーリーを並列で開始可能 (チームの能力が許す場合)
- [P] とマークされたユーザーストーリーのすべてのテストは並列実行可能
...
Task: "tests/contract/test_[name].py の [エンドポイント] の契約テスト"
...
1. フェーズ 1 を完了: セットアップ
2. フェーズ 2 を完了: 基盤 (重要 - すべてのストーリーをブロック)
3. フェーズ 3 を完了: ユーザーストーリー 1
...
- [ストーリー] ラベルはトレーサビリティのためにタスクを特定のユーザーストーリーにマッピング
- 各ユーザーストーリーは独立して完了可能でテスト可能であるべき
- 実装前にテストが失敗することを確認
- 各タスクまたは論理グループの後にコミット
- 任意のチェックポイントで停止してストーリーを独立して検証
- 避けるべきこと: 曖昧なタスク、同じファイルの競合、独立性を壊す横断的なストーリー依存関係
